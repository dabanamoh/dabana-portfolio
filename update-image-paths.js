...
// Function to update the case-studies-data.js file with the data
function updateCaseStudiesDataFile() {
    try {
        // Read the existing case-studies-data.js file
        const filePath = path.join(__dirname, 'js', 'case-studies-data.js');
        let fileContent = fs.readFileSync(filePath, 'utf8');
        
        // Convert the case studies data to a string representation
        const caseStudiesString = JSON.stringify(caseStudiesData, null, 4);
        
        // Replace the caseStudiesData object in the file
        fileContent = fileContent.replace(
            /const caseStudiesData = {[\s\S]*?};/m,
            `const caseStudiesData = ${caseStudiesString};`
        );
        
        // Write the updated content back to the file
        fs.writeFileSync(filePath, fileContent);
        
        console.log('✅ Successfully updated case-studies-data.js with local image paths!');
    } catch (error) {
        console.error('❌ Error updating case-studies-data.js:', error);
    }
}

// Run the update function
updateCaseStudiesDataFile();